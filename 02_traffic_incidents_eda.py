...
# 2. Aggregate incidents by road, count them, and find the date range (Earliest start & Latest end)
road_summary = messages_df.groupby('road').agg(
    Incident_Count=('road', 'size'),
    Earliest_Start=('start_time', 'min'),
    Latest_End=('end_time', 'max')
).reset_index()

# Sort by incident count descending and take top 10
road_summary = road_summary.sort_values(by='Incident_Count', ascending=False).head(10)

# 3. Format timestamps to show only date and hours:minutes (dropping seconds and timezones)
road_summary['Earliest_Start'] = road_summary['Earliest_Start'].dt.strftime('%Y-%m-%d %H:%M')
road_summary['Latest_End'] = road_summary['Latest_End'].dt.strftime('%Y-%m-%d %H:%M')

# 4. Display the clean summary table with dates for our portfolio
print("Top Roads by Traffic Incidents with Date & Time Ranges:")
...
# MAGIC %sql
# MAGIC SELECT DISTINCT DATE(snapshot_date) FROM fintraffic_risk_db.traffic_incidents ORDER BY 1 DESC;